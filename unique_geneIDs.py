...
#results.to_csv(path_or_buf=(result_filename), sep="\t", index=False)
...
### converting the arrays into a list
l = []
s = len(unique_clean)
for i in range(0,s):
    for item in unique_clean[i]:
        l.append(item)

x = np.unique(l)

x = pd.DataFrame(x)
x.to_csv(path_or_buf=("unique_gene_list.txt"), sep="\t", index=False, header = False)
